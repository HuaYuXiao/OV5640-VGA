...
#(
    parameter CMOS_H_PIXEL = 24'd640,   // CMOS图像水平像素数
    parameter CMOS_V_PIXEL = 24'd480    // CMOS图像垂直像素数
    )
(
    input clk,          // 输入时钟
    input rst_n,        // 复位信号
    
    input i2c_done,     // I2C操作完成信号
    output reg i2c_exec,// I2C执行信号
    output reg [23:0] i2c_data, // I2C数据
    output reg init_done // 初始化完成信号
    );
    
localparam REG_NUM = 8'd248;           // 寄存器个数
localparam TOTAL_H_PIXEL = CMOS_H_PIXEL + 13'd1216; // 总的水平像素数
localparam TOTAL_V_PIXEL = CMOS_V_PIXEL + 13'd504;  // 总的垂直像素数
    
reg [14:0] start_init_cnt;  // 开始初始化计数器
reg [7:0] init_reg_cnt;     // 初始化寄存器计数器        

// 寄存器延时
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)                  // 当复位信号rst_n为低电平时
        start_init_cnt <= 15'd0;    // 将计数器 start_init_cnt 初始化为 0
    else if(start_init_cnt < 15'd20000)    // 当计数器 start_init_cnt 小于 20000 时
        start_init_cnt <= start_init_cnt + 1'b1;    // 计数器 start_init_cnt 自增 1
end

// 寄存器个数
always @(posedge clk or negedge rst_n) begin
    if (!rst_n)                      // 当复位信号为低电平时
        init_reg_cnt <= 8'd0;        // 将寄存器个数初始化为0
    else if (i2c_exec)               // 当I2C执行信号为高电平时
        init_reg_cnt <= init_reg_cnt + 8'b1;  // 寄存器个数加1
end

// I2C执行控制
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        i2c_exec <= 1'b0;  // 复位时将I2C执行控制信号置为0
    else if(start_init_cnt == 15'd19999)
        i2c_exec <= 1'b1;  // 当初始计数器达到指定值时，设置I2C执行控制信号为1，开始执行I2C操作
    else if(i2c_done && (init_reg_cnt < REG_NUM))
        i2c_exec <= 1'b1;  // 当前一次I2C操作完成并且初始化寄存器计数器未达到指定数量时，设置I2C执行控制信号为1，继续执行下一次I2C操作
    else
        i2c_exec <= 1'b0;  // 其他情况下，将I2C执行控制信号置为0，停止执行I2C操作
end

// 初始化完成信号
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)                      // 如果复位信号为低电平
        init_done <= 1'b0;          // 初始化完成信号为低电平
    else if((init_reg_cnt == REG_NUM) && i2c_done)   // 如果寄存器计数器达到指定数量且I2C操作完成
        init_done <= 1'b1;          // 初始化完成信号为高电平
end
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)
        ...
        case(init_reg_cnt)
            8'd0  : i2c_data <= {16'h3008,8'h82}; //Bit[7]:澶嶄綅 Bit[6]:鐢垫簮浼戠湢
            8'd1  : i2c_data <= {16'h3008,8'h02}; //姝ｅ父宸ヤ綔妯″紡
            ...
            8'd5  : i2c_data <= {16'h3037,8'h13}; //PLL鍒嗛鎺у埗
            8'd6  : i2c_data <= {16'h3108,8'h01}; //绯荤粺鏍瑰垎棰戝櫒
            ...
            8'd22 : i2c_data <= {16'h3600,8'h08}; //VCM鎺у埗,鐢ㄤ簬鑷姩鑱氱劍
            8'd23 : i2c_data <= {16'h3601,8'h33}; //VCM鎺у埗,鐢ㄤ簬鑷姩鑱氱劍
            8'd24 : i2c_data <= {16'h302d,8'h60}; //绯荤粺鎺у埗
            8'd25 : i2c_data <= {16'h3620,8'h52};
            8'd26 : i2c_data <= {16'h371b,8'h20};
            8'd27 : i2c_data <= {16'h471c,8'h50};
            8'd28 : i2c_data <= {16'h3a13,8'h43}; //AEC(鑷姩鏇濆厜鎺у埗)
            8'd29 : i2c_data <= {16'h3a18,8'h00}; //AEC 澧炵泭涓婇檺
            8'd30 : i2c_data <= {16'h3a19,8'hf8}; //AEC 澧炵泭涓婇檺
            ...
            8'd38 : i2c_data <= {16'h3c06,8'h00}; //light meter 1 闃堝?糩15:8]
            8'd39 : i2c_data <= {16'h3c07,8'h08}; //light meter 1 闃堝?糩7:0]
            8'd40 : i2c_data <= {16'h3c08,8'h00}; //light meter 2 闃堝?糩15:8]
            8'd41 : i2c_data <= {16'h3c09,8'h1c}; //light meter 2 闃堝?糩7:0]
            ...
            8'd48 : i2c_data <= {16'h4001,8'h02}; //BLC(榛戠數骞虫牎鍑?)琛ュ伩璧峰琛屽彿
            8'd49 : i2c_data <= {16'h4005,8'h1a}; //BLC(榛戠數骞虫牎鍑?)琛ュ伩濮嬬粓鏇存柊
            8'd50 : i2c_data <= {16'h3000,8'h00}; //绯荤粺鍧楀浣嶆帶鍒?
            8'd51 : i2c_data <= {16'h3004,8'hff}; //鏃堕挓浣胯兘鎺у埗
            8'd52 : i2c_data <= {16'h4300,8'h61}; //鏍煎紡鎺у埗 RGB565
            8'd53 : i2c_data <= {16'h501f,8'h01}; //ISP RGB
            8'd54 : i2c_data <= {16'h440e,8'h00};
            8'd55 : i2c_data <= {16'h5000,8'ha7}; //ISP鎺у埗
            8'd56 : i2c_data <= {16'h3a0f,8'h30}; //AEC鎺у埗;stable range in high
            8'd57 : i2c_data <= {16'h3a10,8'h28}; //AEC鎺у埗;stable range in low
            8'd58 : i2c_data <= {16'h3a1b,8'h30}; //AEC鎺у埗;stable range out high
            8'd59 : i2c_data <= {16'h3a1e,8'h26}; //AEC鎺у埗;stable range out low
            8'd60 : i2c_data <= {16'h3a11,8'h60}; //AEC鎺у埗; fast zone high
            8'd61 : i2c_data <= {16'h3a1f,8'h14}; //AEC鎺у埗; fast zone low
            //LENC(闀滃ご鏍℃)鎺у埗 16'h5800~16'h583d
            ...
            //AWB(鑷姩鐧藉钩琛℃帶鍒?) 16'h5180~16'h519e
            ...
            //Gamma(浼介┈)鎺у埗 16'h5480~16'h5490
            ...
            //CMX(褰╄壊鐭╅樀鎺у埗) 16'h5381~16'h538b
            ...
            //SDE(鐗规畩鏁扮爜鏁堟灉)鎺у埗 16'h5580~16'h558b
            ...
            //CIP(棰滆壊鎻掑??)鎺у埗 (16'h5300~16'h530c)
            ...
            //绯荤粺鏃堕挓鍒嗛 Bit[7:4]:绯荤粺鏃堕挓鍒嗛 input clock =24Mhz, PCLK = 48Mhz
            8'd203: i2c_data <= {16'h3035,8'h11}; 
            8'd204: i2c_data <= {16'h3036,8'h3c}; //PLL鍊嶉
            8'd205: i2c_data <= {16'h3c07,8'h08};
            //鏃跺簭鎺у埗 16'h3800~16'h3821
            ...
            8'd235: i2c_data <= {16'h4004,8'h02}; //BLC(鑳屽厜) 2 lines
            8'd236: i2c_data <= {16'h4713,8'h03}; //JPEG mode 3
            8'd237: i2c_data <= {16'h4407,8'h04}; //閲忓寲鏍囧害
            ...
            8'd241: i2c_data <= {16'h5001,8'ha3}; //ISP 鎺у埗
            8'd242: i2c_data <= {16'h3b07,8'h0a}; //甯ф洕鍏夋ā寮?  
            //褰╂潯娴嬭瘯浣胯兘 
            8'd243: i2c_data <= {16'h503d,8'h00}; //8'h00:姝ｅ父妯″紡 8'h80:褰╂潯鏄剧ず
            //娴嬭瘯闂厜鐏姛鑳?
            8'd244: i2c_data <= {16'h3016,8'h02};
            8'd245: i2c_data <= {16'h301c,8'h02};
            8'd246: i2c_data <= {16'h3019,8'h02}; //鎵撳紑闂厜鐏?
            8'd247: i2c_data <= {16'h3019,8'h00}; //鍏抽棴闂厜鐏?
            //鍙瀛樺偍鍣?,闃叉鍦╟ase涓病鏈夊垪涓剧殑鎯呭喌锛屼箣鍓嶇殑瀵勫瓨鍣ㄨ閲嶅鏀瑰啓
            default : i2c_data <= {16'h300a,8'h00}; //鍣ㄤ欢ID楂?8浣?
        endcase
    end    
end        
endmodule
