...
wire [3 : 0] m_axi_awid  = 4'd0                     ;
reg [29 : 0] m_axi_awaddr                           ;
reg [7 : 0] m_axi_awlen                             ; 
wire [2 : 0] m_axi_awsize = 3'h2                    ; //每次传输32bit
wire [1 : 0] m_axi_awburst = 2'b01                  ; //increment 模式
wire m_axi_awlock = 1'b0                            ;
wire [3 : 0] m_axi_awcache = 4'b0011                ; //Normal Non-cacheable Bufferable
wire [2 : 0] m_axi_awprot = 3'd0                    ;
wire [3 : 0] m_axi_awqos = 4'd0                     ;
reg m_axi_awvalid                                   ;
wire m_axi_awready                                  ;
wire [31 : 0] m_axi_wdata = wdata                   ;
wire [3 : 0] m_axi_wstrb = m_axi_wvalid ? 4'hf:4'd0   ; //4'hf, 与写数据通道同步
wire m_axi_wlast                                    ;
wire m_axi_wvalid = wdata_valid                     ;
wire m_axi_wready                                   ;
wire [3 : 0] m_axi_bid                              ;
wire [1 : 0] m_axi_bresp                            ;
wire m_axi_bvalid                                   ;
wire m_axi_bready = 1'b1                            ;
wire [3 : 0] m_axi_arid = 4'd0                      ;
reg [29 : 0] m_axi_araddr                           ;
reg [7 : 0] m_axi_arlen                             ;
wire [2 : 0] m_axi_arsize = 3'h2                    ;
wire [1 : 0] m_axi_arburst = 2'b01                  ;
wire m_axi_arlock = 1'b0                            ;
wire [3 : 0] m_axi_arcache = 4'b0011                ;
wire [2 : 0] m_axi_arprot = 3'd0                    ;
wire [3 : 0] m_axi_arqos = 4'd0                     ;
reg m_axi_arvalid                                   ;
wire m_axi_arready                                  ;
wire [3 : 0] m_axi_rid                              ;
...
wire m_axi_rlast                                    ;
wire m_axi_rvalid                                   ;
wire m_axi_rready = master_ready                    ;

wire ui_clk                                         ;
wire ui_clk_sync_rst                                ;
//wire init_calib_complete                            ;
wire clk = ui_clk                                   ;
wire rst_n = init_calib_complete & sys_rst_n        ;

assign wdata_ready = m_axi_wready & (wr_state == WRITE_DATA) && (already_wdata_num < wr_len_total_r);
assign rdata_valid = m_axi_rvalid & master_ready;
